...
func test_can_fall_with_gravity():
	var player_controller: PlayerController = add_child_autofree(PLAYER_CONTROLLER_SCENE.instantiate())
	simulate(player_controller, 1, 2)
	assert_lt(player_controller.position.y, 0.0)
...
func test_can_walk_forward(params = use_parameters([Vector2.UP, Vector2.DOWN])):
	...
	assert_between(player_controller.position.z, min(min_z, max_z), max(min_z, max_z))
...
func test_can_walk_sideways(params = use_parameters([Vector2.LEFT, Vector2.RIGHT])):
	...
	assert_between(player_controller.position.x, min(min_x, max_x), max(min_x, max_x))
...
func test_looking_up_or_down_doesnt_decrease_speed(params = use_parameters([Vector2.UP, Vector2.DOWN])):
	var player_controller: PlayerController = PLAYER_CONTROLLER_SCENE.instantiate()
	...
	simulate(player_controller, 1, 1)
	var distance_when_looking_straight := player_controller.position.length_squared()
	...
	simulate(player_controller, 1, 1)
	var distance_when_looking_not_straight := player_controller.position.length_squared()
...
func test_can_walk_up_slopes():
	...
	var test_map := add_child_autofree(TEST_MAP.instantiate()) as Node3D
	var player_controller := test_map.get_node("Player") as PlayerController
	var pre_ramp_transform := test_map.get_node("RampPositionPre1").transform as Transform3D
	...
	assert_between(
		abs(player_controller.position.x),
...
func test_doesnt_slide_down_walkable_slopes():
	...
	var test_map := add_child_autofree(TEST_MAP.instantiate()) as Node3D
	var player_controller := test_map.get_node("Player") as PlayerController
	var ramp_position_transform := test_map.get_node("RampPositionOn1").transform as Transform3D
	...
	var x_position := player_controller.position.x
...
func test_does_slide_down_unwalkable_slopes():
	...
	var test_map := add_child_autofree(TEST_MAP.instantiate()) as Node3D
	var player_controller := test_map.get_node("Player") as PlayerController
	var ramp_position_transform := test_map.get_node("RampPositionOn2").transform as Transform3D
	player_controller.transform = ramp_position_transform
...
func test_player_cant_walk_up_slopes_that_are_too_steep():
	...
	var test_map := add_child_autofree(TEST_MAP.instantiate()) as Node3D
	var player_controller := test_map.get_node("Player") as PlayerController
	var pre_ramp_transform := test_map.get_node("RampPositionPre2").transform as Transform3D
	player_controller.transform = pre_ramp_transform
